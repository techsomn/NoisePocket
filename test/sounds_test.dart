...
void main() {
  ...
  test('every pad has three bundled, non-silent PCM WAV sounds', () async {
    expect(sounds, hasLength(26));
    expect(sounds.map((s) => s.file).toSet(), hasLength(26));
    expect(sounds.where((s) => s.category == SoundCategory.reactions), hasLength(8));
    expect(sounds.where((s) => s.category == SoundCategory.cartoon), hasLength(6));
    expect(sounds.where((s) => s.category == SoundCategory.arcade), hasLength(6));
    expect(sounds.where((s) => s.category == SoundCategory.world), hasLength(6));
