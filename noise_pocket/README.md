...
Version **2.0.0** keeps the Noise Pocket look: bright orange, purple, blue, yellow, green, and pink pads on charcoal, lime active outline and Surprise Me button, and a dark Playback Settings sheet. The header and icon pads are cleaner, and a fixed status card shows the current sound without shifting the grid. A tap highlights its pad immediately. Surprise Me chooses a different pad when one is already selected; it never changes a sound's pitch. Open settings with the sliders icon beside the logo. Ads are rendered only from AdMob; no ad artwork is hardcoded into the screen.
